...
import { createAssessmentInitializationObserver, getAssessmentInitializationObservabilityConfig, } from "../service/assessmentInitializationObservability.service.js";
function syntheticFailureEvent(requestId) {
    return {
 ...
 */
export async function runSyntheticInitializationFailureCli(dependencies = {}) {
    const writeOutput = dependencies.writeOutput ?? ((value) => console.log(value));
    const writeError = dependencies.writeError ?? ((value) => console.error(value));
    const loadConfig = dependencies.loadConfig ?? getAssessmentInitializationObservabilityConfig;
    const describeDeliveryError = (error) => `Telemetry delivery failed: ${error instanceof Error ? error.message : String(error)}`;
    let config;
    try {
        config = loadConfig();
    }
    ...
    const observer = dependencies.createObserver?.(config) ??
