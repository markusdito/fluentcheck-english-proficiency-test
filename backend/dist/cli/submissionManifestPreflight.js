...
import { inspectSubmissionManifestReadiness, } from "../service/submissionManifestPreflight.service.js";
...
function formatHumanPreflight(result) {
    const lifecycle = result.legacy.lifecycle.length
        ? result.legacy.lifecycle
            .map((row) => `  ${row.status}: ${row.submissionCount} Submissions, ${row.answerCount} Answers`)
            .join("\n")
        : "  none";
...
export async function runSubmissionManifestPreflightCli(args, dependencies = {}) {
