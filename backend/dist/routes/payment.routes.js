...
export function createPaymentRouter(ipaymuTransport = fetchIpaymuTransport) {
    const router = Router();
    router.post("/ipaymu/notify", ipaymuNotification);
    router.post("/submissions/:id/pay", verifyToken, (req, res) => paySubmission(req, res, ipaymuTransport));
    return router;
}
