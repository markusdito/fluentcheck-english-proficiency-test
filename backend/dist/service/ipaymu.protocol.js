...
export function canonicalizeIpaymuCallback(body) {
    const normalized = {};
...
export function classifyIpaymuCheckoutResponse(httpStatus, result) {
    if (httpStatus >= 500)
...
export function createIpaymuCheckoutTimeout(abortController, timeoutMs = IPAYMU_CHECKOUT_TIMEOUT_MS) {
    let timeout;
