...
export async function inspectSubmissionManifestReadiness(client, dependencies = {}) {
    ...
    try {
        const lifecycle = (await client.query(`SELECT s."status"::text AS "status",
          ...
          END`)).rows;
        const duplicateActiveLegacySubmissions = (await client.query(`SELECT ARRAY_AGG(s."id" ORDER BY s."createdAt", s."id") AS "submissionIds"
           FROM "Submission" s
           LEFT JOIN "SubmissionManifest" sm ON sm."submissionId" = s."id"
          WHERE sm."id" IS NULL AND s."status" = 'IN_PROGRESS'
          GROUP BY s."studentId"
         HAVING COUNT(*) > 1
          ORDER BY MIN(s."createdAt"), MIN(s."id"::text)`)).rows;
        const brokenReferences = (await client.query(`SELECT
          ...
          (SELECT COUNT(*)::int FROM "Answer" a
           ...
           WHERE a."manifestEntryId" IS NOT NULL AND me."id" IS NULL)
            AS "manifestAnswersWithoutEntries"`)).rows[0];
        const laterEnforcementViolations = (await client.query(`SELECT
          (SELECT COUNT(*)::int FROM "Submission" s
            LEFT JOIN "SubmissionManifest" sm ON sm."submissionId" = s."id"
           WHERE sm."id" IS NULL AND s."status" = 'IN_PROGRESS')
          ...
          (SELECT COUNT(*)::int
            ...
            WHERE sm."version" = 1
              ...
              )) AS "invalidVersion1Manifests"`)).rows[0];
