import {
  inventoryPromptMedia,
  runPromptMediaCleanup,
  type PromptMediaCleanupInventory,
  type PromptMediaCleanupMode,
  type PromptMediaCleanupOptions,
  type PromptMediaCleanupRunResult,
} from "../service/promptMediaCleanup.service.js";

interface CleanupPromptMediaCliDependencies {
  inventory?: () => Promise<PromptMediaCleanupInventory>;
  run?: (
...
function formatHumanInventory(result: PromptMediaCleanupInventory): string {
...
function parseArguments(args: string[]) {
  let mode: PromptMediaCleanupMode | undefined;
  let json = false;
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === "--json") json = true;
      ...
      if (mode) throw new Error("Choose exactly one of --execute or --finalize");
      mode = "QUARANTINE";
    } else if (argument === "--finalize") {
      if (mode) throw new Error("Choose exactly one of --execute or --finalize");
      mode = "FINALIZE";
    } else if (argument === "--actor-id") actorId = valueAfter(args, index++, argument);
    else if (argument === "--authorization-id") authorizationId = valueAfter(args, index++, argument);
    else if (argument === "--reason") reason = valueAfter(args, index++, argument);
...
function formatHumanRun(result: PromptMediaCleanupRunResult): string {
  return [
    `Prompt-media cleanup ${result.mode.toLowerCase()} run ${result.runId}`,
    `Status: ${result.status}`,
    ...result.objects.map(
      (object) => `${object.storageKey} ${object.status} ${object.outcome}${object.error ? ` - ${object.error}` : ""}`,
    ),
  ].join("\n");
}

/** CLI seam used by the dry-run and explicitly authorized cleanup commands. */
export async function runCleanupPromptMediaCli(
  args: string[],
  dependencies: CleanupPromptMediaCliDependencies = {},
): Promise<number> {
  ...
  try {
    ...
    const result = await (dependencies.run ?? ((mode, options) => runPromptMediaCleanup(mode, options)))(
      parsed.mode,
      {
