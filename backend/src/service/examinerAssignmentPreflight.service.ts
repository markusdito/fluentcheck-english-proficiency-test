...
interface ExaminerAssignmentPreflightDependencies {
  now?: () => Date;
}

interface AssignmentGroupRow extends QueryResultRow {
  submissionId: string;
...
export interface ExaminerAssignmentPreflightResult {
  ...
  conflicts: {
    oneAssignmentSubmissions: number;
    excessAssignmentSubmissions: number;
    unpopulatedSlotAssignments: number;
    invalidSlotAssignments: number;
    duplicateSlotSubmissions: number;
    duplicateExaminerSubmissions: number;
    lifecycleInconsistentSubmissions: number;
  };
  exitCode: 0 | 1;
}
 ...
 */
export async function inspectExaminerAssignmentReadiness(
  client: Client,
  dependencies: ExaminerAssignmentPreflightDependencies = {},
): Promise<ExaminerAssignmentPreflightResult> {
  ...
  try {
    const assignmentGroups = (
      await client.query<AssignmentGroupRow>(SQL.assignmentGroups)
    ).rows;

    await client.query("COMMIT");

    const conflicts = {
      oneAssignmentSubmissions: assignmentGroups.filter(
        (group) => group.assignmentCount === 1,
      ).length,
      excessAssignmentSubmissions: assignmentGroups.filter(
        (group) => group.assignmentCount > 2,
      ).length,
      unpopulatedSlotAssignments: assignmentGroups.reduce(
        (sum, group) => sum + group.unpopulatedSlots,
        0,
      ),
      invalidSlotAssignments: assignmentGroups.reduce(
        (sum, group) => sum + group.invalidSlots,
        0,
      ),
      duplicateSlotSubmissions: assignmentGroups.filter(
        (group) => group.duplicateSlots > 0,
      ).length,
      duplicateExaminerSubmissions: assignmentGroups.filter(
        (group) => group.duplicateExaminers > 0,
      ).length,
      lifecycleInconsistentSubmissions: assignmentGroups.filter(
        (group) => group.lifecycleInconsistent > 0,
      ).length,
    };
