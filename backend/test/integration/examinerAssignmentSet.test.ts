...
test("a repeated attempt after a committed set is an idempotent EXISTING replay", async () => {
  ...
  assert.deepEqual(
    replay.assignments.map((assignment) => assignment.id).sort(),
    first.assignments.map((assignment) => assignment.id).sort(),
  );
  assert.equal(
...
test("concurrent attempts commit exactly one set and converge on the same examiners", async () => {
  ...
  const results = await Promise.allSettled([
    createExaminerAssignmentSet(submission.id, { selectCandidates }),
    createExaminerAssignmentSet(submission.id, { selectCandidates }),
  ]);
  ...
  const fulfilled = results
    .filter(
      (
        ...
        Awaited<ReturnType<typeof createExaminerAssignmentSet>>
  ...
  assert.equal(fulfilled.filter((result) => result.outcome === "EXISTING").length, 1);

  const assignmentIds = new Set(fulfilled[0].assignments.map((assignment) => assignment.id));
  assert.equal(assignmentIds.size, 2);
  assert.deepEqual(
    fulfilled[1].assignments.map((assignment) => assignment.id),
...
test("an injectable selector determines the chosen pair deterministically", async () => {
  ...
  const chosen = result.assignedExaminers.map((examiner) => examiner.id).sort();
